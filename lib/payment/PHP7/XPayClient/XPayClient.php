...
class XPayClient
{
	 ...
	 * @access public
	 */
	function __construct($home_dir, $mode="real")
	{
...
//		$this->log("session id = ".session_id().$_SESSION['tx_counter'], LGD_LOG_FATAL);
	...
	function Log($msg, $level=LGD_LOG_FATAL)
	{
		...
		if (function_exists('mb_detect_encoding')) {
			if (mb_detect_encoding($msg, "EUC-KR, UTF-8") == "UTF-8" && $this->config['output_UTF8'] == 0) {
				$err_msg = date("Y-m-d H:i:s")." [".$this->err_label[$level]."] [".$this->TX_ID."] ".iconv("utf-8", "euc-kr", $msg)."\n";
			} else {
