...
//버튼 변환스크립트
function writeBtn(btnType,chapter,contentsCode,lectureOpenSeq,seq){
	...
	if(btnType == 'study'){	//학습하기 관련
		...
		if(btnType == 'midTestComplete'){
			writes += '<button type="button" title="응시완료" onclick="resultAct(\'test\',\''+contentsCode+'\',\''+lectureOpenSeq+'\',\'mid\',\'C\')">';
		}else{
			writes += '<button type="button" title="응시완료" onclick="resultAct(\'test\',\''+contentsCode+'\',\''+lectureOpenSeq+'\',\'final\',\'C\')">';
		}
	...
	}else if(btnType == 'printPop'){//수료증출력관련
		writes += '<button type="button" title="수료증출력" onclick="printPop('+seq+')">';
		writes += '<img src="../images/study/btn_printPop.png" alt="수료증출력" /></button>';
...
function captchaRun(chkSeq,types,links){
	...
	popOpen = window.open(links,"studyWindow","top=0,left=0,menubar=no,status=no,titlebar=no,toolbar=no,scrollbar=no,resizeable=no","study");
	popOpen.focus();
}
//수강하기
function studyPop(obj,chapter,types,captchChk){
	...
	$.get(chapterApi,{'contentsCode':checkCode,'lectureOpenSeq':checkOpenSeq},function(data){
		...
		if(eval(chapter) > eval(midTerm) && (data.midStatus != 'Y' && data.midStatus != 'C') && (eval(data.serviceType)== 1 || eval(data.serviceType)==9 )){
			...
			var studyPopOpen = window.open(popupAddress,"studyWindow","top=0,left=0,location=yes,menubar=no,status=no,titlebar=no,toolbar=no,scrollbar=no,resizeable=no","study")
				studyPopOpen.focus();
			/*
...
function chkTest(types,obj){
	...
	$.get(chapterApi,{'contentsCode':checkCode,'lectureOpenSeq':checkOpenSeq},function(data){
		...
		if(types == 'mid'){
			if((eval(data.totalProgress) < eval(data.midTestProgress)) && eval(data.midTestProgress) != 0){
...
//평가완료 후 결과창
function resultAct(types,contentsCode,lectureOpenSeq,testType,resultView){
	if(resultView == 'C') {
...
function progressChange(checkSeq,checkCode,checkOpenSeq,checkchapter){
	var check= $.get(chapterApi,'contentsCode='+checkCode+'&lectureOpenSeq='+checkOpenSeq,function(data){
		...
		//진도율에 따른 평가상태 변경
		if(($beforeProgress < data.midTestProgress && data.totalProgress >= data.midTestProgress)||($beforeProgress <= 79 && data.totalProgress >= 80)){
		...
		if($beforeProgress < data.midTestProgress && data.totalProgress >= data.midTestProgress){//평가 응시가능 알림
...
function openDetail(viewCode){
	popOpen = window.open("../study/lecturepop.php?contentsCode="+viewCode,"studyWindow","top=0,left=0,width=680,height=620,menubar=no,status=no,titlebar=no,toolbar=no,scrollbar=no,resizeable=no","study");
	popOpen.focus();
}

function openPopup(linkAddress){
	popOpen = window.open(linkAddress,"studyWindow","top=0,left=0,width=380,height=460,menubar=no,status=no,titlebar=no,toolbar=no,scrollbar=no,resizeable=no","study");
	popOpen.focus();
}
...
function chkTestArea(data,writeTarget){//평가진행 관련 버튼ul
	...
	var midMsg = '';
	var midLink = ''
	if(eval(data.totalProgress) < eval(data.midTestProgress) && eval(data.midTestProgress) != 0 && data.midStatus != null){
		midtext = '진도율 '+data.midTestProgress+'%이상 응시가능합니다.'
		...
		}else if(data.midStatus == 'Y') { //평가응시 후
		...
		}else if(data.midStatus == 'C') { //평가응시 후
			midLink = 'resultAct(\'test\',\''+contentsCode+'\','+lectureOpenSeq+',\'mid\',\'C\')';
			midMsg = '<strong class="blue">응시결과보기</strong>';
			midtext = '응시일 : '+data.midSaveTime.substr(0,10);
		}
	}
	...
	if(data.totalProgress <= 79 && data.testStatus != null){
		...
		}else if(data.testStatus == 'Y') { //평가응시 후
			finLink = 'resultAct(\'test\',\''+contentsCode+'\','+lectureOpenSeq+',\'final\')';
			finMsg = '<strong class="blue">응시완료</strong>';
			fintext = '응시일 : '+data.testSaveTime.substr(0,10);
		}else if(data.testStatus == 'C') { //평가응시 후
			finLink = 'resultAct(\'test\',\''+contentsCode+'\','+lectureOpenSeq+',\'final\',\'C\')';
			finMsg = '<strong class="blue">응시결과보기</strong>';
			fintext = '응시일 : '+data.testSaveTime.substr(0,10);
		}
	}
...
function runStudyList(data,writeTarget){
		...
		if(i != midTerm){
			...
			if(useMidTest != null && (data.serviceType == 1 || data.serviceType == 9 || data.serviceType == 5)){//서비스타입에 따른 노출 지정
				...
				if(data.midStatus == 'Y' || data.midStatus == 'C'){
					studyList += writeBtn('midTestComplete','',''+data.contentsCode+'',''+data.lectureOpenSeq+'','');
				}else{
	...
	if(useFinTest != null && (data.serviceType == 1 || data.serviceType == 9 || data.serviceType == 5 )){ //2017.07.19 강혜림 서비스타입 5번 추가
		...
		if(data.testStatus == 'Y' || data.testStatus == 'C' || (data.testStatus = 'V' && data.nowTime >= data.testEndTime)){
			studyList += writeBtn('finTestComplete','',''+data.contentsCode+'',''+data.lectureOpenSeq+'','');
		}else{
	...
	//수료증 2017.11.20 강혜림 추가
	if(data.passOK == 'Y'){
		studyList += '<tr class="printLine">';
		studyList += '<td>[수료증]</td>';
		studyList += '<th class="blue"><strong>수료증출력</strong>';
		studyList += '<td class="reportStatus"><strong class="red">수료</strong></td>';
		studyList += '<td>-</td>';
		studyList += '<td>'
		studyList += writeBtn('printPop','','','',''+data.seq+'');
		studyList += '</td>'
		studyList += '</tr>';
	}
