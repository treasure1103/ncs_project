...
.printArea > ul li h1:after { content:':' }
.printArea > p { margin-top:2cm; font-size:20pt; line-height:22pt; font-weight:bold; text-align:center; }
...
<?
		...
		if($serviceType == 3) {
			...
			$serviceTypeQ2 = "serviceType='3'";
			$typeText = "인터넷 훈련과정";
		} else {
			$serviceTypeQ = "A.serviceType='1' AND A.passOK='Y'";
			$serviceTypeQ2 = "serviceType='1' AND passOK='Y'";
		...
		$query = "SELECT B.birth, F.companyName, B.department,
			...
			$query2 = " SELECT DISTINCT(contentsCode) FROM nynStudy WHERE ".$serviceTypeQ2." AND companyCode='".$companyCode."'
									AND lectureStart='".$lectureStart."' AND lectureEnd='".$lectureEnd."'";
			$result2 = mysql_query($query2);
			$count2 = mysql_num_rows($result2);
			if($count2 == 1) {
				$contentsText = $rs[contentsName];
			} else {
				$count3 = $count2 - 1;
				$contentsText = $rs[contentsName]." 외 ".$count3."과정";
			}
			if($count != 1) {
				$count4 = $count-1;
				$nameText = " 외 ".$count4."명";
...
?>
				<div class="printArea">
					...
					<ul>
						...
						<li><h1>성&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;명&nbsp;&nbsp;</h1><?=$rs[userName];?><?=$nameText;?></li>
						<li><h1>훈련과정&nbsp;&nbsp;</h1><?=$contentsText;?></li>
